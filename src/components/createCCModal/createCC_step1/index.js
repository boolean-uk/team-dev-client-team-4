...
const CreateCohortCourseModal_step1 = (props) => {
  ...
  >
    ...
    <section>
      ...
      <section className="create-cohort-course-actions">
          ...
          onClick={() => {
              setCreateCC_data({
                ...createCC_data, courseId: selectedCourse, cohortId: selectedCohort, courseName: courseList.find(c => c.id.toString() === selectedCourse)?.name || '',
                cohortName: cohortList.find(c => c.id.toString() === selectedCohort)?.cohortName || '', startDate: selectedCohortObj?.startDateFormatted || '', endDate: selectedCohortObj?.endDateFormatted || ''
              });
              setStep(2);
            }}
            />
        ...
        {step === 2 && (
      <CreateCohortCourseModal_step2 createCC_data={createCC_data} />
    )}
        </Card>
  );
};
